...
function getCompanyBySlug(equals: string, locale: Locale) {
    return getMultipleCompanies({
        locale,
        page: 1,
        limit: 1,
        where: { slug: { equals } },
...
export default async function Page({ params: { companyName, locale } }: Props) {
    ...
    } = await getCompanyBySlug(companyName, locale)
...
export async function generateStaticParams(args: Props) {
    const {
        Companies: { docs },
    ...
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- autogenerated type, slug will be there
